...
class HD_WP_Metabox_API {
	...
	function __construct( $options = array(), $fields = array() ) {
		...
		$this->supported_fields = (array) apply_filters( 'hd_metabox_api_supported_fields', array( 'text', 'textarea', 'radio', 'checkbox', 'select', 'multicheck', 'upload', 'color', 'editor', 'img_radio', 'html', 'register_only' ) );
	...
	function enqueue_styles_scripts( $hook_suffix ) {
		...
		wp_enqueue_script( 'hd-html-helper', WPSL_ROOT_URL . 'admin/js/admin.js', array( 'jquery', 'wp-color-picker', 'select2' ), null, true );
	...
	function display_metabox( $post ) {
		...
		echo '<div class="hd-metabox-inner wpsl_metabox ' . sanitize_html_class( $this->options['metabox_id'] ) . '">';
